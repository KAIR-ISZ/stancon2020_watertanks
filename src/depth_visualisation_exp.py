...
if __name__ == '__main__':
    ...
    beta_fault = fit_fault1.extract('beta').get('beta')
    y_healthy = fit_healthy.extract('y_pred').get('y_pred')
    y_pred_ft = fit_fault1.extract('y_pred').get('y_pred')
...
# number of samples per trajectory
    ...
    df = pd.read_csv('../data/result/result_med_600.csv')
    ...
    for i in range(num_knots+2):
        ax_spl.plot(trajectories1.time.values[0:M],(spline_components[:, i]), color=DARK, zorder=0)
    ...
    ax_y.set_xticklabels(['0 s', '300 s', '600 s'])
